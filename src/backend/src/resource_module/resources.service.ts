...
export class ResourcesService {
    ...
    for (const resource of resources) {
      if (resource.type === ImageType) {
        ...
        try {
          ...
          console.error(`Failed to retrieve image from Yandex Disk for resource ${resource._id}: ${error.message}`);
        }
      }
  ...
  async findOne(id: string): Promise<ResourceDto> {
    const existingResource = await this.resourceModel.findOne({ _id: id }).exec();
  ...
  async create(resourceDto: CreateUpdateResourceDto): Promise<ResourceDto> {
    ...
    if (resourceDto.type === ImageType) {
      ...
      try {
        ...
        await client.putFileContents(remotePath, imageBuffer, { overwrite: true });
  ...
  async update(id: string, resourceDto: CreateUpdateResourceDto): Promise<ResourceDto> {
    ...
    if (resourceDto.type === ImageType) {
      ...
      try {
        if (existingResource.value && existingResource.value.includes(PUBLIC_YANDEX_DISK_URL)) {
        ...
        throw new NotFoundException(
          `Failed to update image on Yandex Disk: ${uploadError.message}`,
        );
      }
    }
  ...
  async delete(id: string): Promise<void> {
    ...
    if (existingResource.type === ImageType && existingResource.value.includes(PUBLIC_YANDEX_DISK_URL)) {
      try {
        const parts = existingResource.value.split('/');
        const filename = decodeURIComponent(parts[parts.length - 1]);
        const remotePath = `${YANDEX_DISK_FOLDER}/${filename}`;
